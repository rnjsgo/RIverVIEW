...
def get_keyword(df: pd.DataFrame, product_name, col = 'review', size = _DEFAULT_SIZE_):
    ...
    for sentence in data_frame[col]:
        tokenized_senetence = mecab_pos(sentence)
        text = ''
        for word, tag in tokenized_senetence:
            if word in word2id:
                if (tag in get_words) and not_useless(word) and (tfidf[i, word2id[word]] > _IMPORTANCE_):
                    text += (word +' ')
        if text:
    ...
    if not view_single:
        for review in tqdm(df[col]):
            ...
            for w in keyword: #문장에 키워드를 포함하지 않는 키워드는 일단 보지않음.
                word = get_contain_keyword(w, review)
        ...
        for review in tqdm(df[col]):
            ...
            for sentence in review_list:
                for w in keyword: #문장에 키워드를 포함하지 않는 키워드는 일단 보지않음.
                    word = get_contain_keyword(w, sentence)
                ...
                for sentence in reveiw_sentence:
                    if not find_example:
                        ...
                        if key != '_etc':
                            ...
                            if abs(key_score[key], sentence_score) < 5:
                                find_example=True
                                example_sentece[w] = {
